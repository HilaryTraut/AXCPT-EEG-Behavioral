...
def probe(probeFile,lst,indx):
    ...
    posResp.autoDraw = False; negResp.autoDraw = False; probeImage.autoDraw = False #Draw images
    myWin.flip()
    probeStart = core.getTime()                                                  #Set probeStart timestamp after window flip

    k = event.waitKeys(maxWait = 1.5, timeStamped = False)                        #Probe display time: 1500ms or until keypressed
...
def snoozeSlide():
    ...
    snooze.autoDraw = False           #present snooze image
...
def respFeedback(k, lst, indx):
    ...
    if k[0][0] == lst[indx][2]:               #Does keypress match correct answer?
        posFeed.autoDraw = False               #Draw positive feedback
        acc = 1                               #Set response accuracy
        
    elif k[0][0] != lst[indx][2]:             #Does keypress mismatch correct answer?
        negFeed.autoDraw = False               #Draw negative feedback
...
def instruct():
    ...
    instruct1 = visual.TextStim(win = myWin, text = 'You are going to see some pictures of animals. All the animals travel in pairs, one after the other.',
                                    pos = (0,0.5), height = 0.05, wrapWidth = 1.5, color = 'black'); instruct1.autoDraw = False
    ...
    instruct2 = visual.TextStim(win = myWin, text = 'The Duck and the Pig are good friends, so if you see a Duck followed by a Pig press the food button so they can eat together!\n\nIf you see any other animal pairs, press the X.',
                                    pos = (0,-0.2), height = 0.05, wrapWidth = 1.5, color = 'black'); instruct2.autoDraw = False
...
def prac():
    ...
    pracText.autoDraw = False; myWin.flip()                 #Display practice instructions.
    ...
    while pracTrial < len(pracList) - 1:                   #Con't administering practice until no more trials available.
        repeatText = visual.TextStim(win = myWin, text = 'Press SPACE to go on. Press any other key to try again.',
                                             pos = (0,0), height = 0.05, wrapWidth = 1.5, color = 'black'); repeatText.autoDraw = False; myWin.flip()
    ...
    pracText.setText('Great work! Ready to play for real?'); pracText.autoDraw = False; myWin.flip()   #Notify that prac as ended.
...
def setTrig(trig_name):
    ns.send_event('evt_', label="event", timestamp=egi.ms_localtime(),   #EGI trigger
                          table = {'fld1' : trig_name})
...
header = "part_id" + ","+ "dob" + ", "+ "age_group" + ","+ "runner" + "," + "date" + "," + "block" + "," \
...
######################################################################
#Presentation screen
myWin = visual.Window(size = (1000,800), fullscr = False,
                          winType = 'pyglet', color = 'black') 
...
while block < 11:                                         #Set number of blocks to admin.
    ...
    while trialnum != len(trialList):                                                          #Administer trials within the block
        ns.sync()
        ns.send_event('evt_', label="event", timestamp=egi.ms_localtime(),                     #EEG: trigger start
              table = {'fld1' : "BeginTrial"})                              
        ...
        ns.send_event('evt_', label="event", timestamp=egi.ms_localtime(),
                          table = {'fld1' : "EndTrial"})                                       #EEG: trigger end
